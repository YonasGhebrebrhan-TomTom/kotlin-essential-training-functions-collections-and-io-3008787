...
1. **Do not write the exercise code.** Not a scaffold, not "just the boilerplate," not a
   corrected version pasted back. Yonas types the Kotlin. If asked for a hint, give the
...
3. **Review only after it's written**, against the instructor's end state
   (`git diff HEAD origin/0X_0Ye -- project/src/`), and explain *why* the two differ.
...
- `origin/*b` / `origin/*e` — the course's 78 snapshots. **Never check these out**; they ship
